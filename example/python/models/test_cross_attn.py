    ...
    for i in range(loop):
        ...
        for j in range(max_seq_len - seq_len):
            res_eet = eet_model(input_ids=input_ids, attention_mask=attention_mask,
                                first_pass=first_pass, self_past_key_values_length=self_past_key_values_length)
            self_past_key_values_length += input_ids.shape[1]
            print(j, ' res eet: ', res_eet, ' shape: ', res_eet.shape)
            if first_pass:
                first_pass = False
...
def test_cross_attn():
    ...
    eet_model = EETBartModel.from_pretrained('facebook/bart-base', batch_size, seq_len, data_type=data_type)
    eet_cross_attn = eet_model.decoder.layers[0].cross_attention

    self_attn_out = torch.from_numpy(np.random.random((batch_size, 1, 768))).to(data_type).cuda()
    self_attn_out_inc = torch.from_numpy(np.random.random((batch_size, 1, 768))).to(data_type).cuda()
    # encoder_out = torch.empty(0)
    encoder_out = torch.from_numpy(np.random.random((batch_size, seq_len, 768))).to(data_type).cuda()
    pre_padding_len = torch.empty(0)
    encoder_padding_mask = torch.empty(0)

    res = eet_cross_attn(input_id=self_attn_out, pre_padding_len=pre_padding_len,
                         encoder_out=encoder_out, encoder_padding_mask=encoder_padding_mask, first_pass=True)
    print('eet first cross attn out: ', res.size())
    # res = eet_cross_attn(input_id=self_attn_out_inc, pre_padding_len=pre_padding_len,
    #                      encoder_out=torch.empty(0), encoder_padding_mask=encoder_padding_mask, first_pass=False)    



if __name__ == '__main__':
    test(batch_size=1, seq_len=10, max_seq_len=12, using_half=False,
