...
def test(batch_size=4, seq_len=8, max_seq_len=1024, using_half=True, using_eet=True, using_ts=True, loop=100):
    ...
    # load model
        ...
        for i in range(loop):
            ...
            first_pass = True
            for j in range(max_seq_len - seq_len):
                res_eet = eet_model(input_ids=input_ids, decoder_input_ids=input_ids, attention_mask=attention_mask, first_pass=first_pass, self_past_key_values_length=self_past_key_values_length)
...
if __name__ == '__main__':
    test(batch_size=4, seq_len=10, max_seq_len=14, using_half=True,
