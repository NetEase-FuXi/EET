...
def make_batches(lines, cfg, task, max_positions, encode_fn):
    tokens = [
        task.target_dictionary.encode_line(
            encode_fn(src_str), add_if_not_exist=False,append_eos=False,reverse_order=True
        ).long()
    ...
    yield Batch(ids=torch.arange(len(tokens)),
                src_tokens=tokens, 
...
def main(cfg: FairseqConfig):
    ...
    eet_model = EETTransformerDecoder.from_torch(model_id_or_path = cfg.common_eval.path,
                                                dictionary = src_dict,args = model_cfg, 
                                                config = eet_config,
