...
class EETXLMRobertaModel():
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type, device_id=0):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaModel.from_pretrained(model_id_or_path)
        cfg = torch_model.config
        model_name = 'xlm_roberta'  # cfg.model_type
        ...
        eet_model = EETXLMRobertaModel(cfg, embedding, encoder, torch_model.pooler)
        return eet_model
    ...
    def from_torch(torch_model, max_batch, data_type, device_id=0):
        ...
        if data_type == torch.float32:
            ...
            torch_model = torch_model.half()
        eet_model = EETXLMRobertaModel(cfg, embedding, encoder, torch_model.roberta.pooler)
        return eet_model


class EETXLMRobertaForCausalLM():
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForCausalLM.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        lm_head = torch_model.lm_head.cuda()
        model = EETXLMRobertaForCausalLM(roberta, lm_head, torch_model.config)

        return model


class EETXLMRobertaForMaskedLM():
    def __init__(self, roberta, lm_head, config):
        self.config = config
        self.roberta = roberta
        self.lm_head = lm_head
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForMaskedLM.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        lm_head = torch_model.lm_head.cuda()
        model = EETXLMRobertaForMaskedLM(roberta, lm_head, torch_model.config)

        return model


class EETXLMRobertaForSequenceClassification():
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForSequenceClassification.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        classifier = torch_model.classifier.cuda()
        model = EETXLMRobertaForSequenceClassification(roberta, classifier, torch_model.config)

        return model


class EETXLMRobertaForMultipleChoice():
    def __init__(self, roberta, classifier, config):
        self.config = config
        self.roberta = roberta
        self.classifier = classifier
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForMultipleChoice.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        classifier = torch_model.classifier.cuda()
        model = EETXLMRobertaForMultipleChoice(roberta, classifier, torch_model.config)

        return model


class EETXLMRobertaForTokenClassification():
    def __init__(self, roberta, classifier, config):
        self.config = config
        self.roberta = roberta
        self.classifier = classifier
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForTokenClassification.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        classifier = torch_model.classifier.cuda()
        model = EETXLMRobertaForTokenClassification(roberta, classifier, torch_model.config)
        return model


class EETXLMRobertaForQuestionAnswering():
    ...
    def from_pretrained(model_id_or_path: str, max_batch, data_type):
        ...
        model_dict = {}
        embedding_dict = {}
        torch_model = XLMRobertaForQuestionAnswering.from_pretrained(model_id_or_path)
        roberta = EETXLMRobertaModel.from_torch(torch_model, max_batch, data_type)
        qa_outputs = torch_model.qa_outputs.cuda()
        model = EETXLMRobertaForQuestionAnswering(roberta, qa_outputs, torch_model.config)
        return model
