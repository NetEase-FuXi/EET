...
namespace eet{
    namespace op{
        ...
        void MultiHeadAttention::layer_norm(const torch::Tensor& input, Buffer& layernorm_query)
        {
            ...
            if (with_bias_)
            {
                RUN_KERNEL(layernorm, desc_.dtype_, input.data_ptr(), layernorm_weights_, layernorm_bias_, layernorm_query.data_ptr(), m, n, desc_.layernorm_eps_, desc_.stream);
            }
            else
            {
                RUN_KERNEL(T5layernorm, desc_.dtype_, input.data_ptr(), layernorm_weights_, layernorm_query.data_ptr(), m, n, desc_.layernorm_eps_, desc_.stream);
            }
#ifdef _DEBUG_MODE_
