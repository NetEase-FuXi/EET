...
namespace eet{
    namespace op{
        LlamaMmha::LlamaMmha(MetaDesc desc,
            ...
            layernorm_bias_(layernorm_bias.data_ptr())
        {
            ...
            // MManager::get_instance().get_cache(desc_.hidden_units_ * desc_.hidden_units_ * 3, desc_.dtype_, desc_.options_, "weight_cache");
        ...
        torch::Tensor LlamaMmha::forward(torch::Tensor& input,
        ...
        // full decoder
        torch::Tensor LlamaMmha::forward_full(torch::Tensor &input,
                                              const torch::Tensor &pre_padding_length,
                                              bool pre_layernorm,
                                              bool add_residual,
                                              const torch::Tensor &relative_attention_bias)
        {
            assert((input.dtype() == desc_.dtype_) && "input's dtype is not the same as LlamaMmha's dtype");
            step_ = 1;
            ...
            // print_to_screen((half*)qkv_buffer.data_ptr(),32);

            // qkv transpose & qk rotary embedding
            ...
            fused_RoPE_transpose(qkv_buffer, q_buf, k_buf, v_buf, -1);
        ...
        // incremental decoder
        torch::Tensor LlamaMmha::forward_inc(torch::Tensor &input,
                                             const torch::Tensor &pre_padding_len,
                                             const torch::Tensor &reorder_state,
                                             bool pre_layernorm,
                                             bool add_residual,
                                             const torch::Tensor &relative_attention_bias)
        {
            ...
            // print_to_screen((half*)qkv_buffer.data_ptr(),8);
            ...
            Buffer& v_buf = MManager::get_instance().get_buffer(desc_.batch_size_ * cur_seq_len_ *
                                    inner_dim_, desc_.dtype_, desc_.options_, false, "q_inc");
            fused_RoPE_transpose(qkv_buffer, q_buf, k_buf, v_buf, step_ - 1);
        ...
        // layerNorm
        void LlamaMmha::layer_norm(const torch::Tensor& input_tensor, Buffer& layernorm_query)
        {
        ...
        void LlamaMmha::qkv_weights_mul(void* input, 
                                    Buffer& qkv_buffer){
                const int m = cur_batch_size_ * cur_seq_len_;
        ...
        void LlamaMmha::qkv_weights_mul_weight_only_int8(void *input, Buffer &qkv_buffer)
        {
        ...
        void LlamaMmha::fused_RoPE_transpose(const Buffer &qkv_buffer,
                                             Buffer &q_buf,
                                             Buffer &k_buf,
                                             Buffer &v_buf,
                                             int time_step)
        {
            RUN_KERNEL(fused_QKV_bias_rotary_kernel,desc_.dtype_,qkv_buffer.data_ptr(),q_bias_,
                       k_bias_, v_bias_, q_buf.data_ptr(), k_buf.data_ptr(), v_buf.data_ptr(),
                       cur_batch_size_, cur_seq_len_, desc_.head_num_, size_per_head_, time_step, desc_.stream);
#ifdef _DEBUG_MODE_
            cudaDeviceSynchronize();
            check_cuda_error(cudaGetLastError());
#endif
        ...
        void LlamaMmha::q_k_mul(const Buffer& q_buf,
                                const Buffer& k_buf,
                                Buffer& qk_buf){
    ...
    check_cuda_error(cudaGetLastError());
        ...
        void LlamaMmha::add_relative_attn_bias(Buffer &qk_buf, void *relative_attention_bias)
        {
        ...
        void LlamaMmha::qk_softmax(Buffer& qk_buf,const int64_t *padding_len){
    ...
    check_cuda_error(cudaGetLastError());
        ...
        void LlamaMmha::attn_v_mul(const Buffer& qk_buf,
                                   const Buffer& v_buf,
    ...
    check_cuda_error(cudaGetLastError());
        ...
        void LlamaMmha::transpose(const Buffer& transpose_dst, Buffer&  dst){
        ...
        void LlamaMmha::project(const Buffer &dst, Buffer &res, torch::Tensor &input, bool pre_layernorm, bool add_residual)
        {
            const int m = cur_batch_size_ * cur_seq_len_;
            int k = inner_dim_;
            int n = desc_.hidden_units_;
        ...
        void LlamaMmha::kv_transpose(torch::Tensor& d_K_buf, torch::Tensor& d_V_buf,Buffer& K_buf,Buffer& V_buf)
        {
        ...
        void LlamaMmha::fused_masked_attention(const Buffer &qkv_buffer,
                                               Buffer &context_buf,
        ...
        void LlamaMmha::reorder_cache(torch::Tensor &K_cache, torch::Tensor &V_cache, Buffer &K_buf, Buffer &V_buf, const int64_t *reorder_index)
        {
        ...
        void LlamaMmha::reorder_cache(torch::Tensor &K_cache, torch::Tensor &V_cache, torch::Tensor &K_buf, torch::Tensor &V_buf, const int64_t *reorder_index)
        {
            RUN_KERNEL(reorderKV_kernel, desc_.dtype_, K_cache.data_ptr(), V_cache.data_ptr(), K_buf.data_ptr(), V_buf.data_ptr(), reorder_index, first_batch_size_, step_, desc_.head_num_, size_per_head_);
#ifdef _DEBUG_MODE_
            cudaDeviceSynchronize();
        ...
        void LlamaMmha::masked_attention(const Buffer &q_buf,
                                         const Buffer &k_buf,
                                         const Buffer &v_buf,
                                         Buffer &context_buf,
                                         const int64_t *padding_len,
